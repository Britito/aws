...
from flask import flash # es el encargado de mostrar mensajes y errores (VALIDACION)
...
class User:
    ...
    def valida_usuario(formulario):
        ...
        #validamos que el nombre, apellido y password tenga al menos 3 0 6caracteres 
        #registro palabra inventada par ausar en usuarios controllers  en l ruta
        ...
        #verificar contrseñas coincidan

        if (formulario['password'] != formulario['confirm_password']):
            flash('Contraseñas no coinciden', 'registro')
            es_valido = False
    
        #verificamos email con expresiones regulares
        if not EMAIL_REGEX.match(formulario['email']):
            flash('Email inválido', 'registro')
            es_valido = False

        #consultemos si existe el correo electrónico
    ...
    @classmethod
    def save(cls,formulario):   # primeras llaves coinciden con columnas y la interpolacion con el formualrio
        ...
        return result #nos regresa el id del nuevo registro que se regreso
